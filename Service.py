...
def bestfriend_effect(engine, hero):
    hero.hp = hero.max_hp
    engine.hero = Objects.BestFriend(hero)
    engine.notify("Your Bro helped you!")
...
def add_gold(engine, hero):
    if random.randint(1, 10) == 1:
        ...
        engine.score -= 0.05
        engine.hero = Objects.Poisoning(hero)
...
class EmptyMap(MapFactory):
    ...
    class Map:
        def __init__(self):
            self.Map = [[0 for _ in range(13)] for _ in range(13)]
            for i in range(13):
                for j in range(13):
    ...
    class Objects:
        ...
        def get_objects(self, _map):
            ...
            for i in range(random.randint(prop['min-count'], prop['max-count'])):
                coord = (random.randint(1, 12), random.randint(1, 12))
                intersect = True
                while intersect:
                    intersect = False
                    if _map[coord[1]][coord[0]] == wall:
    ...
    class Objects:
        ...
        def get_objects(self, _map):
            ...
            for obj_name in self.config:
                if obj_name not in object_list_prob['enemies']:
                    continue
...
def service_init(sprite_size, full=True):
    ...
    object_list_actions = {'reload_game': reload_game,
                           ...
                           'bestfriend_effect': bestfriend_effect}
